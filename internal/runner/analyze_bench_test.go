...
func realScenarioIterationBytes(b *testing.B) [][]byte {
	...
	for i := range 3 {
		...
		data, err := os.ReadFile(p) //nolint:gosec // G304: path from filepath.Join
		if err != nil {
...
func BenchmarkAnalyze_RealThreeIterations(b *testing.B) {
	...
	for b.Loop() {
		...
		_, _, cleanup, err := Analyze(rs, benchSlowThreshold)
		if err != nil {
			b.Fatal(err)
		}
		if cleanup != nil {
...
func BenchmarkAnalyzeResults_RealDir(b *testing.B) {
	...
	if _, _, cleanup, err := AnalyzeResults(dir, benchSlowThreshold); err != nil {
	...
	b.SetBytes(total)
	for b.Loop() {
		_, _, cleanup, err := AnalyzeResults(dir, benchSlowThreshold)
		if err != nil {
			b.Fatal(err)
		}
		if cleanup != nil {
