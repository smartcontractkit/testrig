...
func TestWilsonScoreInterval(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.GreaterOrEqual(t, lower, 0.0, "lower must be >= 0")
			assert.LessOrEqual(t, upper, 1.0, "upper must be <= 1")
			if tc.n > 0 {
...
func TestReportSummary_hasCI(t *testing.T) {
	...
	assert.GreaterOrEqual(t, *s.FlakeExecutionFailRateLower, 0.0)
	assert.LessOrEqual(t, *s.FlakeExecutionFailRateUpper, 1.0)
	...
	assert.GreaterOrEqual(t, *s.FlakeIterationFailRateLower, 0.0)
	assert.LessOrEqual(t, *s.FlakeIterationFailRateUpper, 1.0)
