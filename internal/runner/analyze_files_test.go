...
`
	...
	b, err := os.ReadFile(filepath.Join(dir, rel)) //nolint:gosec // G304: path from filepath.Join
...
`,
	...
	b0, err := os.ReadFile(filepath.Join(dir, "logs/p_T_iter-0.log")) //nolint:gosec // G304: path from filepath.Join
	require.NoError(t, err)
...
func TestDiagnoseLogFilenameLongIterationStaysWithinLimit(t *testing.T) {
	...
	iter := strings.Repeat("1234567890", 10)
