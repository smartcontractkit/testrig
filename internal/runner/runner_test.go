...
func TestDiagnoseCanceledCtxAIStdoutCompleteEvent(t *testing.T) {
	...
	err := Diagnose(
		ctx,
		conf,
		output.NewForTest(conf.AIOutput, &stdout, io.Discard, false),
		[]string{"./..."},
		nil,
		nil,
		nil,
...
func TestParseDiagnoseGoTestCount(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "count after -args ignored",
			args:    []string{"-v", "-args", "-count=50"},
			wantSet: false,
			wantN:   0,
			wantErr: false,
		},
...
func TestRunDiagnoseIterationsDumpDBSkippedWhenCancelled(t *testing.T) {
	...
	_, err := runDiagnoseIterations(
		ctx, conf,
		output.NewForTest(conf.AIOutput, io.Discard, io.Discard, false),
		t.TempDir(),
		[]string{"./..."},
		[]diagnoseIterationResource{{DumpDiagnostics: dumpDB}},
		diagnoseRunHooks{},
	)
	require.NoError(t, err)
...
func TestPrintDiagnoseResultsDirHeader(t *testing.T) {
	...
	t.Run("ai", func(t *testing.T) {
		t.Parallel()
		var stdout, stderr strings.Builder
		p := output.NewForTest(true, &stdout, &stderr, false)
		printDiagnoseResultsDirHeader(p, dir)
...
func TestRunDiagnoseIterationsRunsInParallelWithWorkerIsolation(t *testing.T) {
	...
	var stdout strings.Builder
	out := output.NewForTest(true, &stdout, io.Discard, false)
	...
	resources := []diagnoseIterationResource{
		{
			Env: []string{"TEST_WORKER=worker-0"},
		...
		{
			Env: []string{"TEST_WORKER=worker-1"},
			Reset: func(context.Context) error {
				mu.Lock()
				defer mu.Unlock()
				resets++
	...
	hooks := diagnoseRunHooks{
		runIteration: func(_ context.Context, p diagnoseIterationParams) error {
			...
			err := os.WriteFile(
				filepath.Join(p.ResultsDir, "iteration-"+strconv.Itoa(p.Iteration)+".log.jsonl"),
	...
	for _, env := range envByIter {
		assert.Len(t, env, 1)
		assert.Contains(t, env[0], "TEST_WORKER=worker-")
	}
		...
		runIteration: func(ctx context.Context, p diagnoseIterationParams) error {
			...
			if p.Iteration == 0 {
				require.NoError(
					t,
					os.WriteFile(
						filepath.Join(p.ResultsDir, "iteration-0.log.jsonl"),
						[]byte(`{"Action":"fail","Package":"p","Test":"T","Elapsed":0.01}`+"\n"),
						0600,
					),
				)
...
func TestShouldFailFastIterationOptimization(t *testing.T) {
	...
	failed, reason := shouldFailFastIteration(conf, nil, IterationDigest{}, nil)
...
func TestDiagnoseBuildErrorStopsWithoutAnalysis(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var stderr strings.Builder
			out := output.NewForTest(tc.aiOutput, io.Discard, &stderr, false)
...
func TestRunDiagnoseIterationsCallsIterationHooks(t *testing.T) {
	t.Parallel()
	resultsDir := t.TempDir()
	conf := &config.App{
		RepoRoot:           t.TempDir(),
	...
	var setupBeforeTeardown atomic.Bool // true once setup is seen before any teardown

	hooks := diagnoseRunHooks{
		iterationSetup: func(_ context.Context) error {
			if teardownCalls.Load() == setupCalls.Load() {
				setupBeforeTeardown.Store(true)
			}
			setupCalls.Add(1)
			return nil
		},
		iterationTeardown: func(_ context.Context) error {
			teardownCalls.Add(1)
			return nil
		},
		runIteration: func(_ context.Context, p diagnoseIterationParams) error {
