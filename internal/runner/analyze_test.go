...
func readLogMapContent(t *testing.T, path string) string {
	t.Helper()
	if path == "" {
		return ""
	}
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantOutput, readLogMapContent(t, logs[tc.wantKey][tc.wantIter]))
		})
	}
}
...
`
	...
	for _, e := range rep.Timeouts {
		...
		assert.Contains(t, readLogMapContent(t, logs[k][0]), "panic: test timed out after 5s")
	}
}
...
func TestPrintSummaryPackageLevelFlakeDoesNotPrintPackageAsTest(t *testing.T) {
	...
	rep := &Report{
		...
		Flakes: []TestEntry{
			...
			{
				Package: "github.com/smartcontractkit/chainlink/v2/core/services/vrf/v2",
				Test:    "TestVRFV2Integration_SingleConsumer_ForceFulfillment",
				Runs:    48,
				Fails:   1,
			},
		},
...
func TestAnalyzeResultsRoundtrip(t *testing.T) {
	...
	b, err := os.ReadFile(filepath.Join(dir, "report.json")) //nolint:gosec // G304: path from filepath.Join
	require.NoError(t, err)
