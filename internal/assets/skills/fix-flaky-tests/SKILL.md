...
description: >-
  Diagnoses and fixes unstable Go tests (flakes, races, timeouts, deadlocks, slow).
---
...
- DO NOT use for full-suite CI prep.
...
<cli_reference>
`testrig -h`
`testrig diagnose [harness_flags] -- [go_test_flags] ./path` (same `./path` as above).
...
2. Read `.agents/skills/fix-flaky-tests/CONTEXT.md` if it exists to gather repository-specific context, test conventions, and guidelines.
3. Read `.agents/skills/fix-flaky-tests/custom-init.md` if it exists and execute any additional initialization steps defined there.
4. Check if you have access to the `trunk` MCP server. If not, prompt the user to [install and authenticate it](https://github.com/trunk-io/mcp-server#quick-start).
  <trunk>
...
4. Investigate and understand the specific test code.
5. Formulate initial hypothesis based on any data you can gather from the user or code.
</init>

<loop>
1. Read `.agents/skills/fix-flaky-tests/references/custom-loop.md` if it exists and apply any additional loop instructions or constraints defined there.
2. If a `diagnose-attempted-fixes-[test/package]-[flake/broken/timeout/slow].jsonl` file exists, read it to see previous fix attempts and findings. If not, create one.
3. Form a hypothesis on the cause of the issues
3. Implement a fix
...
8. If you successfully resolve a flaky test, synthesize the root cause and the fix applied into a reusable pattern. Save this as a new markdown file in `.agents/skills/fix-flaky-tests/references/known-patterns/<pattern-name>.md`. You MUST include YAML frontmatter with `name:` and `symptoms:` keys so that future runs can scan the metadata efficiently.
...
<known_patterns>
Files in the `.agents/skills/fix-flaky-tests/references/known-patterns/` dir.

During initialization, use shell commands (e.g. `head -q -n 10 .agents/skills/fix-flaky-tests/references/known-patterns/*.md`) to read ONLY the YAML frontmatter of these files. Evaluate the `symptoms:` to see if they match the current failure. DO NOT read the full file contents unless the frontmatter strongly suggests a match.
</known_patterns>
...
1. System Prompt: "You are a headless, read-only log parser. Your sole purpose is to read Go test logs from the end up. Read the logs and construct possible reasons why the test [input reason we're investigating]. You do not converse. You output raw JSON and nothing else."
